...
CUDA is opt-in **per hot-loop library**: only `meridian_map` links CUDA/nvblox. The front-end uses **OpenMP** to parallelise the scan-to-map association across cores for the Jetson Orin budget. We keep accelerator flags library-local so each module's parallelism stays contained.
...
- **Each stage is single-threaded internally** (its module is *not* required to be thread-safe; it is *thread-confined*). Concurrency lives only in the queues. The simplest model that keeps the front-end easy to reason about. A stage MAY own internal parallelism *behind* its interface — nvblox launches GPU kernels from its single map thread, and the LIO front-end runs its scan-to-map association across cores with OpenMP — provided its external surface stays thread-confined. The front-end's deskew and GN solve run sequentially on T2; only the association is parallel, and its floating-point reduction order varies with the thread schedule (§11.2).
...
- **Replay mode:** for tests and bag replay, `pipeline.mode: replay` (§8.2) runs the whole pipeline synchronously on the caller's thread — no stage threads, no queues — so the stage call order is fixed and the run is clock-free. Stage interfaces are identical; only the executor differs. The front-end is thread-confined at the stage level **in both modes** (the factory's `deterministic` flag is a no-op for it, §5.1). GPU kernels that reduce non-deterministically run in their deterministic variant where available.
...
8. **Threading is fixed stages + bounded queues,** front-end prioritized, lossless keyframe/map queues, synchronous single-thread replay mode for tests. (§11)
