...
- **Determinism:** replay==replay is **byte-identical** (fixed cadence + fixed factor-insertion
  order, GTSAM TBB-off, deterministic max-clique budget). Live==replay agreement is *statistical*
  (different batch boundaries). The one nondeterminism caveat is the front-end ikd-Tree async
  rebuild — an L2 property, **not L3** (see §9).
...
|---|---|---|---|---|---|
| **L5 self-test (Stage D)** | single loop | Whitened χ² of one GICP loop vs the corrected odometry chain between *its own* two endpoints. Rejects loops inconsistent with plausible drift. | `meridian_place` `pcm_self_test.cpp` | `place.pcm_chi2_conf` (0.99) | on (`place.pcm`=true) |
...
timing and breaks bit-exact replay).
...
- **Determinism property:** replay==replay is **byte-identical**. Live==replay is *statistical*
  (different batch boundaries → different intermediate linearization points), evaluated per spec 10.
  The max-clique budget is an expansion **count**, not wall-clock, so it never breaks reproducibility.
- **ikd-Tree caveat:** the one known nondeterminism source is the **front-end** ikd-Tree async
  lazy-rebuild — an **L2 property, not L3**. It is why `place.enable` is kept off in the
  bit-exact-replay configs; the back-end optimizer itself is deterministic.
