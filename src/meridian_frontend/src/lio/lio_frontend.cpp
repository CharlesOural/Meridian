...
void LioFrontEnd::ingest(const PreprocessedGroup& group) {
  try {
    ...
    if (!tracker_.initialized()) {
      ...
      if (held_.size() > kMaxHeldGroups) {
        ...
        if (!held_drop_warned_ && telemetry_ != nullptr &&
            telemetry_->enabled("frontend/lio/init_backlog")) {
          telemetry_->event(Level::Warn, "frontend/lio/init_backlog",
    ...
    if (telemetry_ != nullptr) {
      telemetry_->event(Level::Error, "frontend/lio/error", e.what(), group.group.t_end);
    }
  }
}

void LioFrontEnd::processGroup(const MeasureGroup& g) {
  MERIDIAN_SCOPED_TIME(telemetry_, "frontend.lio.ingest", g.t_end);
  ...
  if (have_prev_group_ && to_seconds(g.t_begin - prev_t_end_) > cfg_.lio.max_gap_s) {
    ...
    reseed_armed_ = true;
    if (telemetry_ != nullptr && telemetry_->enabled("frontend/lio/gap")) {
      telemetry_->event(Level::Warn, "frontend/lio/gap",
  ...
  if (!bootstrapped_) {
    if (!scan_usable) {
      if (telemetry_ != nullptr && telemetry_->enabled("frontend/lio/reject")) {
        telemetry_->event(Level::Warn, "frontend/lio/reject",
...
void LioFrontEnd::publishSweepTelemetry(const MeasureGroup& g, const MotionPrior& prior,
  ...
  scal("frontend/assoc/n_attempted", static_cast<double>(n_keypoints));
  scal("frontend/assoc/n_matched", static_cast<double>(res.n_corr));
  scal("frontend/solver/gn_iters", static_cast<double>(res.gn_iters));
  scal("frontend/solver/dx_norm", res.dx_norm);
  scal("frontend/solver/chi", res.chi);
  scal("frontend/map/voxels", static_cast<double>(map_.voxel_count()));
  scal("frontend/map/points", static_cast<double>(map_.size()));
  scal("frontend/state/vel_norm", last_state_.v_world.norm());
  scal("frontend/state/bias_gyr_norm", tracker_.bias_gyro().norm());
  scal("frontend/state/bias_acc_norm", tracker_.bias_accel().norm());
  ...
  const double beta = (cfg_.lio.min_beta > 0.0 && prior.imu_count > 1)
                          ...
                          : -1.0;
  scal("frontend/lio/beta", beta);
  scal("frontend/lio/accel_var", prior.accel_mag_variance);
  scal("frontend/lio/n_corr", static_cast<double>(res.n_corr));
  scal("frontend/lio/deskew_span_t_ms", to_seconds(g.scan.sweep_duration) * 1e3);
  if (telemetry_->enabled("frontend/obs")) {
    Eigen::Matrix<double, 6, 1> ov;
    for (int i = 0; i < 6; ++i) {
      ov[i] = last_obs_.score[static_cast<std::size_t>(i)];
    }
    telemetry_->vec("frontend/obs", ov, t, "tx,ty,tz,rx,ry,rz");
  }
  if (telemetry_->enabled("frontend/obs_min")) {
    telemetry_->scalar("frontend/obs_min",
...
void LioFrontEnd::publishPathSample(const Pose& T_world_body, Timestamp t) {
  if (telemetry_ == nullptr || cfg_.debug_path_sample_hz <= 0.0 ||
      !telemetry_->enabled("frontend/path_sample")) {
    return;
  }
  ...
  telemetry_->pose("frontend/path_sample", T_world_body, Frame::Odom, t);
  last_path_t_ = t;
}
